...
func EventAppid(ctx *iris.Context) {
	...
	log.Info(fmt.Sprintf(">>>>>>>>>>>>>>>>>>>>>>>>> appid:%s\t APPID：%s", appid, APPID))
	// if appid == APPID {
...
func process(ctx *iris.Context, cb Callback) {
	if cb.MsgType == "event" {
		log.Info(">>>>>>>>>>>>>>>>>>>>>>>>> event")
...
func replyEventMessage(ctx *iris.Context, content, toUserName, fromUserName string) {
	...
	xmlcontent := fmt.Sprintf(XmlContentFormat, fromUserName, toUserName, createtime, content)
...
func resetAccessToken() {
	...
	mp["component_appid"] = APPID
	mp["component_appsecret"] = APPSECRET
	mp["component_verify_ticket"] = cache.Tools.GetCVT()
	// 请求接口
	log.Info(fmt.Sprintf(">>>>>>>>>>>>>>>>>>>>>>>>> GET_API_COMPONENT_TOKEN_URL：%s", GET_API_COMPONENT_TOKEN_URL))
	// str, err := tutils.HttpRequestPost(GET_API_COMPONENT_TOKEN_URL, mp)
	log.Info(fmt.Sprintf(">>>>>>>>>>>>>>>>>>>>>>>>> resetAccessToken 请求参数：%+v", mp))
	str, err := tutils.HttpRequestPostJson(GET_API_COMPONENT_TOKEN_URL, mp)
...
func replyApiTextMessage(componentAccessToken, authCode, fromUserName string) {
	...
	// 请求接口
	log.Info(fmt.Sprintf(">>>>>>>>>>>>>>>>>>>>>>>>> GET_API_QUERY_AUTH_URL：%s", GET_API_QUERY_AUTH_URL))
	str, err := tutils.HttpRequestPostJson(fmt.Sprintf(GET_API_QUERY_AUTH_URL, cache.Tools.GetCAT()), mp)
	// str, err := tutils.HttpRequestPost(fmt.Sprintf(GET_API_QUERY_AUTH_URL, cache.Tools.GetCAT()), mp)
	log.Info(fmt.Sprintf(">>>>>>>>>>>>>>>>>>>>>>>>> str：%s", str))
	log.Info(fmt.Sprintf(">>>>>>>>>>>>>>>>>>>>>>>>> err：%+v", err))
	...
	if err == nil && ai.AuthorizationInfo.AuthorizerAccessToken != "" {
		...
		str, err := tutils.HttpRequestPostJson(fmt.Sprintf(SEND_URL, ai.AuthorizationInfo.AuthorizerAccessToken), mp)
		// str, err := tutils.HttpRequestPostJson(fmt.Sprintf(SEND_URL, cache.Tools.GetCAT()), mp)
		log.Info(fmt.Sprintf(">>>>>>>>>>>>>>>>>>>>>>>>> str：%s", str))
		log.Info(fmt.Sprintf(">>>>>>>>>>>>>>>>>>>>>>>>> err：%+v", err))
	}
}
